...
from app.middleware.rate_limit import _proxy_aware_key, limiter
...
def test_two_proxies_skip_two_hops(monkeypatch):
    ...
    assert _proxy_aware_key(req) == "192.0.2.5"
...
def test_missing_header_falls_back_to_socket(monkeypatch):
    ...
    assert _proxy_aware_key(req) == "10.9.8.7"


def test_limiter_is_wired_to_proxy_aware_key():
    assert limiter._key_func is _proxy_aware_key

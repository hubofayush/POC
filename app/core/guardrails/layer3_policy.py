...
class ConsentGuard(BaseGuardrail):
    ...
    Default is deny: when GUARDRAIL_CONSENT_DEFAULT_GRANTED is false (prod
    default), the consent_granted key MUST be present and True. When the
    flag is true (legacy/dev), an absent key is treated as granted but an
    explicit False is always blocked.
    """
    name = "consent"
